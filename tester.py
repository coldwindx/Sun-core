...
def precision(tp, tn, fp, fn):
    return tp / (1e-9 + tp + fp)
def recall(tp, tn, fp, fn):
    return tp / (1e-9 + tp + fn)
def fpr(tp, tn, fp, fn):
    return fp / (1e-9 + fp + tn)
...
def test(ckpt):
    ...
    for epoch in range(13, 60):
        ...
        print(f"{ckpt}\t{tp}\t{tn}\t{fp}\t{fn}\t{acc}\t{pre}\t{rec}\t{fprv}\t{auc}")
...
def main(ckpt):
    ...
    ckpt = f"epoch=59-step=37500.ckpt"
    pretrained_filename = os.path.join(CHECKPOINT_PATH, ckpt)
    classifier = ScPredictor.load_from_checkpoint(pretrained_filename)
    classifier.eval()
    ...
    fprv = fpr(tp, tn, fp, fn)
    auc = 2 * pre * rec / (pre + rec)
    print(f"{tp}\t{tn}\t{fp}\t{fn}\t{acc}\t{pre}\t{rec}\t{fprv}\t{auc}")
